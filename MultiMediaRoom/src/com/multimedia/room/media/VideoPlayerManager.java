 ...
 */
public class VideoPlayerManager implements MediaPlayer.OnCompletionListener,
    ...
    private static class SingletonHolder {
        public static VideoPlayerManager instance = new VideoPlayerManager();
    }
    ...
    public static VideoPlayerManager getInstance(Context context) {
        if (context != null && SingletonHolder.instance.mContextRef == null) {
            ...
            mSystemWriter = new SystemWriter(context);
        }
    ...
    /**
     * Change the SurfaceHolder to display the video in a different surface
     * view.
     * 
     * @param sh the SurfaceHolder to use for video display
     */
    public void changeDisplay(SurfaceHolder sh) {
        if (DEBUG)
            Log.d(TAG, "change surface holder " + sh);
        if (sh != null) {
            mSurfaceHolder = sh;
        }
...
//            mCurrentIndex = mFileList.size();
    ...
    public boolean onError(MediaPlayer mp, int what, int extra) {
        if ( mOnStateListener != null) {
            mOnStateListener.onError(what, extra);
        }
        if (DEBUG)
